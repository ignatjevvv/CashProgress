...
function parseObjectRate(data) {
    // progressBar.rateSallUSD = data._items.rate.sell.value;

    // console.log(data._items[0].rate.sell.value);
    progressBar.rateSallUSD = data._items[0].rate.sell.value;
    
    // for (let item in data._items) {
    //     if (data._items[item].data.currency === 'usd') {
    //         progressBar.rateSallUSD = data._items.rate.sell.value;
    //     }
    // }
