  ...
  let progressBar = {
    ...
    renderInfo() {
      ...
      rateSellUsd.innerHTML = this.rateSallUSD
        ? Math.round(parseFloat(this.rateSallUSD) * 100) / 100
        : '-';
    ...
    convertToUSD() {
      return this.rateSallUSD
        ? (this.totalSum / this.rateSallUSD).toFixed(2)
        : '-';
    },
    ...
    renderHistory() {
      ...
      arrHistory.forEach(item => {
        ...
        let icon =
          item.typeOperation == '+' ? 'right-down-line' : 'left-up-line';
  ...
  function getCurrencyCourseDollarAPI() {
    ...
    fetch(url)
      .then(response => {
        ...
        throw new HttpError(
          `Ошибка сетевого доступа, курс валют недоступен: ${response.status}`,
        );
      })
      .then(data => {
